...
moduleForComponent('date-format-lt', 'helper:date-format-lt', {
  integration: true
});

test('when date is missing', function (assert) {
  this.render(hbs`{{date-format-lt}}`);
...
test('when date is undefined', function (assert) {
  ...
  this.render(hbs`{{date-format-lt date}}`);
...
test('when custom formatted', function (assert) {
  ...
  this.render(hbs`{{date-format-lt date timeZoneName="short"}}`);
