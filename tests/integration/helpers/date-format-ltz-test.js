...
moduleForComponent('date-format-ltz', 'helper:date-format-ltz', {
  integration: true
});

test('when date is missing', function (assert) {
  this.render(hbs`{{date-format-ltz}}`);
...
test('when date is undefined', function (assert) {
  ...
  this.render(hbs`{{date-format-ltz date}}`);
...
test('when custom formatted', function (assert) {
  ...
  this.render(hbs`{{date-format-ltz date timeZone="Pacific/Honolulu"}}`);
