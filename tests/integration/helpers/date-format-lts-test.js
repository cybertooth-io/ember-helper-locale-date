...
moduleForComponent('date-format-lts', 'helper:date-format-lts', {
  integration: true
});

test('when date is missing', function (assert) {
  this.render(hbs`{{date-format-lts}}`);
...
test('when date is undefined', function (assert) {
  ...
  this.render(hbs`{{date-format-lts date}}`);
...
test('when custom formatted', function (assert) {
  ...
  this.render(hbs`{{date-format-lts date timeZoneName="short"}}`);
