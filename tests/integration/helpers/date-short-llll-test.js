...
moduleForComponent('date-short-llll', 'helper:date-short-llll', {
  integration: true
});

test('when date is missing', function (assert) {
  this.render(hbs`{{date-short-llll}}`);
...
test('when date is undefined', function (assert) {
  ...
  this.render(hbs`{{date-short-llll date}}`);
...
test('when custom formatted', function (assert) {
  ...
  this.render(hbs`{{date-short-llll date timeZoneName="short"}}`);
